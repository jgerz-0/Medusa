...
def _create_finding(
    ...
    with session_factory() as session:
        target = Target(
            name="GitHub Ticket Target",
            scope="github-ticket.example.com",
        ...
        scan = Scan(
            ...
            scanner="nuclei",
            status="completed",
            initiated_by="github-ticket-test",
            parameters={"profile": "ticketing"},
        )
        session.add(scan)
        session.flush()

        evidence_payload = {"proof": "github-ticket"}
        finding = Finding(
            scan_id=scan.id,
            title="Synthetic GitHub ticketable finding",
            severity="high",
            cve_id=None,
            description="GitHub ticket dispatch regression fixture",
            metadata_json={"vector": "GET /tickets/github"},
...
def test_create_github_ticket_enforces_rbac(
    ...
    response = client.post(
        ...
        json={
            ...
            "title": "Denied access should be audited",
        },
        headers=headers,
    )
    ...
    with session_factory() as session:
        ...
        assert snapshot.get("resource_id") == "/tickets/github"
        assert missing in snapshot.get("missing_roles", [])


@pytest.mark.parametrize(
    "repository",
    ["invalid", "owner//repo", "own er/repo", "owner/repo#1"],
)
def test_create_github_ticket_rejects_invalid_repository(
    api_client: Tuple[TestClient, InMemoryQueue, sessionmaker[Session], Settings],
    repository: str,
) -> None:
    client, queue, session_factory, _settings = api_client
    ...
    response = client.post(
        ...
        json={
            ...
            "title": "Invalid repository should fail",
        },
        headers=auth_headers(),
    )
...
def test_create_github_ticket_rejects_finding_outside_scope(
    api_client: Tuple[TestClient, InMemoryQueue, sessionmaker[Session], Settings]
) -> None:
    client, queue, session_factory, _settings = api_client
    ...
    response = client.post(
        ...
        json={
            "finding_id": finding_id,
            "repository": "medusa/security",
            "title": "Scope enforcement regression",
        },
        headers=auth_headers(),
    )
...
def test_create_github_ticket_records_hash_and_audit(
    api_client: Tuple[TestClient, InMemoryQueue, sessionmaker[Session], Settings]
) -> None:
    ...
    response = client.post(
        ...
        json={
            "finding_id": finding_id,
            "repository": "Medusa/Security",
            "title": "Queue GitHub issue for dispatcher",
            "body": "Ensure repository normalization",
        },
        headers=auth_headers(),
    )
    ...
    assert job_payload == {"ticket_id": ticket_id, "integration": "github"}
    ...
    with session_factory() as session:
        ...
        assert record.payload["repository"] == "medusa/security"

        audit_entry = (
            session.query(AuditLog)
            .filter(
                AuditLog.action == "create_github_ticket",
        ...
        assert snapshot.get("repository") == "medusa/security"


def test_create_github_ticket_surfaces_queue_failures(
    api_client: Tuple[TestClient, InMemoryQueue, sessionmaker[Session], Settings]
) -> None:
    client, queue, session_factory, _settings = api_client
    finding_id = _create_finding(session_factory)
    ...
    response = client.post(
        ...
        json={
            "finding_id": finding_id,
            "repository": "medusa/security",
            "title": "Queue failure should bubble up",
        },
        headers=auth_headers(),
    )

    assert response.status_code == status.HTTP_503_SERVICE_UNAVAILABLE
    assert response.json()["detail"] == "Failed to enqueue GitHub ticket for dispatch"
    assert not queue.messages
